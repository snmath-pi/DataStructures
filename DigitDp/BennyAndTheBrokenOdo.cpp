// problem: https://www.hackerearth.com/problem/algorithm/benny-and-the-broken-odometer/?purpose=login&source=problem-page&update=google
#include <algorithm>
#include <stack>
#include <array>
#include <bitset>
...
ll dp[12][2][2];
ll dfs(string s, ll pos = 0, ll tight = 1, ll flag = 0) {
    if(pos == s.length()) {
        return flag == 1;
    }
    ...
    if(tight) {
        ...
        for(int i=0; i<=s[pos]-'0'; i++) {
            ll flg = flag;
            if(i == 3) flg = 1;
            if(i == s[pos]-'0') res += dfs(s, pos + 1, 1, flg);
            else res += dfs(s, pos + 1, 0, flg);
        }
