...
int main() {
    ...
    dfs(0, -1, 0);
    debug(par) debug(depth)
    vi p1, p2;
    p1.pb(x), p2.pb(res);
    while(x!=res) {
        if(depth[x] < depth[res]) p2.pb(res=par[res]);
        else if(depth[x]>depth[res]) p1.pb(x=par[x]);
        else if(depth[x]==depth[res]) p1.pb(x=par[x]), p2.pb(res=par[res]);
    }
    reverse(all(p2));
    p2.erase(p2.begin());
    ...
    tsi s(all(p1));
    vi ans(n, 0);
    function<void(int, int, int, int)> gset = [&](int u, int p, int org, int d) {
        if(s.find(u)!=s.end()&&u!=org) return;
        debug(u);
        chkmax(ans[u], d);
        for(auto &v: g[u]) {
            if(v!=p) gset(v, u, org,d+1);
        }
    };
    rep(i, sz(p1)) {
        ans[p1[i]] = max(i, sz(p1)-1-i);
        gset(p1[i], -1, p1[i], ans[p1[i]]);
    }
    debug(ans)
