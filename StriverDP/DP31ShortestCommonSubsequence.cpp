...
int main() {
    ...
    string s, t;
    cin >> s >> t;
    // shortest supersequence is a shortest string such that it contains both s & t
    ...
    int i = n, j = m;
    while(i > 0  && j > 0) {
        if(s[i-1] == t[j-1]) s1 += s[i-1], i--, j--;
        else {
            if(dp[i-1][j] > dp[i][j-1]) s1 += s[i-1], i--;
            else s1 += t[j-1], j--;
        }
    }
    while(i > 0) s1 += s[i-1], i--;
    while(j > 0) s1 += t[j-1], j--;
    reverse(all(s1));
    cout << s1 << nl;
