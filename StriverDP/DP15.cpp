...
int dfs(int id, int k, vvi &dp, vi &a) {
    if(k == 0) return 1;
    if(id == 0) return a[id] == k;
    if(dp[id][k] != -1) return dp[id][k];
    int take = 0; if(k >= a[id]) take = dfs(id-1, k-a[id], dp, a);
    int not_take = dfs(id-1, k, dp, a);
    return dp[id][k] = take | not_take;
}
int main() {
#ifndef ONLINE_JUDGE
    ...
    int k = accumulate(all(a), 0) / 2;
    vvi ndp(n, vi (k + 1, -1));
    cout << "Recursion: ";
    cout << dfs(n-1, k, ndp, a) << nl;
    ...
    vector<int> dpp(k + 1, 0), ndpp(k + 1, 0);
    dpp[0] = 1;
    if(a[0] <= k) dpp[a[0]] = 1;
    rng(i, 1, n) {
        ndpp[0] = 1;
