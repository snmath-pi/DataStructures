...
ll dfs(int id, vl &a, int target, vl &dp) {
    if(id == 0) {
        if(target % a[id] == 0) return target / a[id];
        else return 1e12;
    }
    if(target == 0) return 0;
    if(dp[target] != 1e12) return dp[target];

    ll take = 1e12;
    if(a[id] <= target) take = 1 + dfs(id, a, target-a[id], dp);
    return dp[target] = min(dfs(id-1, a, target, dp), take);
}
int main() {
#ifndef ONLINE_JUDGE
    ...
    vl dp(target + 1, 1e12);
    ll ans = dfs(n-1, a, target, dp);
    cout << (ans == 1e12 ? -1 : ans) << nl;
