...
ll dfs(ll id, vl &w, vl &v, ll weight, vvl &dp) {
    if(id <= 0) {
        if(w[0] <= weight) return v[0];
        else return 0;
    }
    if(dp[id][weight] != -1) return dp[id][weight]; 

    ll not_take = dfs(id-1, w, v, weight, dp);
    ll take = -1e12;
    if(weight >= w[id]) take = dfs(id-1, w, v, weight - w[id], dp) + v[id];
    return dp[id][weight] = max(take, not_take);
}
int main() {
#ifndef ONLINE_JUDGE
    ...
    repn(i, n) cin >> w[i];
    repn(i, n) cin >> v[i];
    vvl dp(n + 1, vl(weight + 1, -1));

    cout << dfs(n, w, v, weight, dp);
