...
int main() {
    ...
    rep(i, n) cin >> a[i];
    // ~ memoization
    // vector<vvl> dp(n, vvl(2, vl(3, -1)));

    // cout << dfs(0, 1, 2, dp, a, n);

    // ~tabulation
    // vector<vvl> dp(n + 1, vvl(2, vl(3, 0)));
    ...
    //             ll profit = 0;
    //             if(j == 1) {
    //                 profit = max(-a[i] + dp[i+1][0][k], dp[i+1][1][k]);
    //             } else {
    //                 profit = max(a[i] + dp[i+1][1][k-1], dp[i+1][0][k]);
    //             }
    //             dp[i][j][k] = profit;
    //         }
    //     }
    ...
    // ~space optimizzation
    vvl dp(2, vl(3, 0)), ndp(2, vl(3, 0));
    per(i, n) {
        rep(j, 2) {
            repn(k, 2) {
                ll profit = 0;
                if(j == 1) {
                    profit = max(-a[i] + dp[0][k], dp[1][k]);
                } else {
                    profit = max(a[i] + dp[1][k-1], dp[0][k]);
                }
                ndp[j][k] = profit;
            }
