...
ld median(vi &a, vi&b) {
    ...
    int start = 0, end = n, ok = (sz(a) + sz(b)) % 2;
    int tot = (n + m + 1) >> 1; // ceil value
    while(start <= end) {
        int mid = start + end >> 1;
        int la = mid, lb = tot - mid;
        int w = la ? a[la-1] : -1e9, x =  lb ? b[lb-1] : -1e9;
        int y = la < n ? a[la] : 1e9, z = lb < m ? b[lb] : 1e9;
        debug(mpr(w, x)); debug(mpr(y,z));
        if(w <= z && y >= x) {
            if(ok) {
                debug(max(w,x));
                return max(w, x);
            } else {
                debug(max(w, x)); debug(min(y, z));
                return (max(w, x) + min(y, z)) / 2.0;
            }
        } else if(w > z) {
            end = mid - 1;
        } else start = mid + 1;
...
int main() {
    ...
    while(i < n) c.pb(a[i++]);
    debug(c);
    if(sz(c) % 2) cout << c[sz(c) / 2] << nl;
    else cout << (ld) (c[sz(c) / 2] + c[sz(c) / 2 - 1] ) / 2.0 << nl;
