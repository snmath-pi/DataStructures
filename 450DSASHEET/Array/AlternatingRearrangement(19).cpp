...
void calcnaise(vi &a) {
    int n = sz(a);
    vi b, c;
    rep(i, n) {if(a[i] < 0) b.pb(a[i]); else c.pb(a[i]);}
    int cnt = 0;
    int i, j;
    for(i = 0, j = 0; i < sz(b) && j < sz(c); i++, j++) {
        a[cnt] = b[i], a[cnt + 1] = c[j];
        cnt += 2;
        
    }
    while(i < sz(b)) a[cnt++] = b[i++];
    while(j < sz(c)) a[cnt++] = c[j++];
}
int main() {
#ifndef ONLINE_JUDGE
    ...
    cin.tie(0);
    cin >> n;
    vi a(n); int neg = 0, pos = 0;
    rep(i, n) cin >> a[i], neg += a[i] < 0, pos += a[i] > 0;
    vi b = a;
    calcnaise(a);
    for(auto &x: a) cout << x << sp; cout << nl;
    calc(b);
